...
class Minter:
    def __init__(self, minter):
        self.update_initial_storage(minter=minter)
    ...
    def set_minter(self, params):
        """(Admin only) Set the contract minter."""
        sp.verify(self.is_administrator(sp.sender), message="FA2_NOT_ADMIN")
...
class MintFungibleNFT:
    """(Mixin) Non-standard `mint` entrypoint for FA2Fungible with incrementing
    id.

    Requires the `Admin` mixin.
    Requires the collection_registry
    """

    @sp.entry_point
    def mint(self, batch):
        """Admin and minter can mint tokens."""
        sp.set_type(batch, sp.TList(sp.TRecord(
        ...
        sp.verify(
            self.is_minter(sp.sender) | self.is_administrator(sp.sender),
            "FA2_NOT_ADMIN_OR_MINTER"
        )
